...
tools it unlocks. Hardware-verified live today operating two real MikroTik
PtP links (an 8.8km management-path link and a 120m link): DISC Lite5 ac
and LHG XL 5 ac (IPQ4019, ROS 7.21.5, nv2 PtP). Full write-up in the new
`docs/api-notes-wireless-rf.md`. 114 tools total, 100% line coverage
maintained.**

**`arm_dead_man` / `cancel_dead_man` (`guard.py`) - the headline feature.**
A reusable, generic anti-lockout primitive - **not** wireless-specific -
validated against the single highest-risk case in this project's own
fleet: an 8.8km PtP link that is the *only* management path to the far-end
device. Before a risky remote write, `arm_dead_man` arms a scheduler
**local to the target device** (`/system/scheduler add`) that reverts the
change and removes itself once it fires - a ONE-SHOT schedule at an
explicit future clock time, not a recurring `interval`:
```
/system scheduler add name=<name> start-date=<date> start-time=<time> \
  interval=00:00:00 policy=read,write,test,policy,reboot \
  on-event=":log warning ...; <revert-commands>; \
             /system scheduler remove [find name=\"<name>\"]"
```
`<date>`/`<time>` are computed as `arm-time + minutes` (1-60, default 3),
read from the TARGET DEVICE's own `/system/clock`, never this host's -
`guard._dead_man_deadline`, real `datetime`/`timedelta` arithmetic, correct
across midnight/month/year rollovers. **Pre-release design correction**
(caught in a hardening pass before this version shipped, documented in full
in `docs/api-notes-wireless-rf.md`): an earlier iteration used an
`interval`-only recurring schedule instead, on the belief (from a first,
mistaken hardware reading) that it fired immediately on arm - re-verified
filtering the device's own log by `topics=script,warning` and found that
claim false; `interval`-only fires correctly at arm-time+interval, not
immediately. The one-shot design was kept anyway, for the *correct* reason:
RouterOS aborts an on-event script entirely at its first unparseable
statement, so a broken revert command means the self-remove never runs
either - an `interval`-only schedule in that situation stays armed and
keeps **re-firing the same broken on-event every interval, indefinitely**;
a one-shot schedule fires exactly once no matter what happens inside it.
`policy=` is now set explicitly too, rather than relying on the device's
own scheduler default (verified permissive enough today, but the one
scheduler entry whose entire job is self-healing a lockout should not
depend on a device-side default).

If the write breaks reachability, the device heals itself with **zero**
further action once its deadline is reached - the on-event script runs
entirely on-device, independent of whether the API session (or anything
else) can still reach it afterward. `name` is always generated by
`arm_dead_man` itself (`"deadman-<10 lower-case hex chars>"`, never
caller-supplied) and returned as the handle `cancel_dead_man` needs;
`cancel_dead_man`'s `name` parameter is validated against that exact shape
(`validate_dead_man_name`), so it can **never** be pointed at an unrelated
scheduler entry (e.g. an admin's own `"backup-daily"` task) - a safety
boundary enforced before the device is ever read, not just a format check.
`revert_commands` (`validate_revert_commands`/`validate_revert_command`) is
any non-empty list (max 10 items) of RouterOS script statements a caller
already built from known-good, previously-read device state -
**structurally restricted, not merely denylisted, to that "restore" shape**:
every item must match `/<path> set [find <field>="<value>"] <field>=<value>
...` (`_REVERT_COMMAND_SET_FIND_RE`) - `set` is the only verb ever
accepted, `[find <field>="<value>"]` (a single selector with a *balanced*
quote/bracket) is mandatory, never a bare `.id`. This replaced a first-pass
denylist of 5 dangerous verbs after an opus re-review found the denylist
insufficient: `/ip/route remove [find ...]` (removes the only route back
across a management-path PtP link), `/ip/address remove [find ...]`, and
`/ip/firewall/filter remove [find ...]` all passed the denylist, as would
an unbalanced-quote parse-abort vector (`/interface/wireless set
[find name="x`) that corrupts the on-event script without tripping any
character/verb check. The structural allowlist closes all of these at
once. Also still hardened against a fixed set of script-structure-breaking
characters (`$ ; \` \ { }` - deliberately NOT `"`/`[`/`]`, which the
package's own `[find name="X"]` idiom needs) and, as a redundant extra
layer under the structural allowlist, the original denylist of dangerous
verbs (`/system reboot`, `/system backup load`, `/user`,
`/system reset-configuration`, `/system routerboard`) - `revert_commands`
restores state, it is not a generic command-execution channel. A
documented limitation (not fixed speculatively): a revert needing to
RECREATE a removed resource (`add`, not `set`) isn't expressible today - no
current caller needs it; see `ROADMAP.md`. Both primitives go through the
exact same allowlist + read-only gate + confirm/preview + audit machinery
as every other guarded write in this package (new `WritePreview.dead_man`
field, additive, `None` for every write before this release). No new
"generic run any command" surface: the on-event script this package ever
builds is a fixed template (`guard._build_dead_man_script`) around
caller-supplied revert statements, never an arbitrary command dispatched
through `ALLOWLIST`.

**Also verified this round: librouteros write persistence.** Every write
in this package goes through `Path.update()`/`.add()`/`.remove()`
(`client.py`), the calls that actually persist a change - confirmed the
callable `Path("set", ...)` form (a lazy no-op unless iterated) is never
used for a write anywhere in this codebase.

**Wireless RF tuning (`/interface/wireless`) - three guarded writes, one
read, closing the specific lockout risk `ROADMAP.md`'s "Richer ROS7 Wi-Fi"
item named as the reason channel/regulatory-domain changes were previously
excluded.**
- `set_wireless_channel(interface_name, frequency, channel_width=None, ...)`
  - **LOCKOUT-RISK**: arms a dead-man by default (`arm_deadman=True`,
    `deadman_minutes` default 3) whose revert restores the interface's
    prior frequency/channel-width, read before anything changes - raises
    (`DeviceCommandError`) rather than arm a PARTIAL revert if
    `channel_width` was requested but the interface's current state has no
    `channel-width` to revert to. The preview's `warning` always reports
    whether the target frequency needs a DFS Channel Availability Check
    under the interface's *current* `frequency-mode`
    (`guard._dfs_preview_warning`) - CONFIRMED LIVE:
    `frequency-mode=superchannel` means no DFS/CAC at all (instant
    switch); outside superchannel, ~60s CAC for most of the 5250-5725MHz
    DFS range, ~600s specifically for the 5600-5650MHz weather-radar
    sub-band (matches RouterOS's own `skip-dfs-channels=10min-cac` naming
    for that exact sub-band). `deadman_minutes` is now enforced against
    that same CAC estimate (`ValidationError` if too short, e.g. the
    default 3min against the weather-radar band's ~10min CAC - a shorter
    window could revert the channel while it's still mid-CAC); the warning
    also flags when the dead-man's own revert target is itself
    DFS-governed.
- `set_wireless_tx_power(interface_name, tx_power, ...)` - forces
  `tx-power-mode=all-rates-fixed`. **LOCKOUT-RISK**, same dead-man default,
  reverting BOTH `tx-power-mode` and `tx-power` together - raises
  (`DeviceCommandError`) rather than fabricate a `"default"`/`"0"` fallback
  if either field is missing from the interface's current state. CONFIRMED
  LIVE: on a short link, the *default* (maximum) tx-power SATURATED the
  receiver (-27dBm measured) and produced a WORSE CCQ (34) than a lower
  explicit power (~8dBm gave -47dBm, CCQ 94) - there is no single "right"
  power, this tool applies whatever the caller supplies. Preview/apply
  both carry a `warning` that CCQ/rate transiently dip for a few seconds
  after applying (rate re-adaptation) - expected, not a failure.
- `set_wireless_tuning(interface_name, adaptive_noise_immunity=None,
  distance=None, arm_deadman=True, deadman_minutes=3, ...)` -
  `adaptive_noise_immunity` alone is reception-only tuning, CONFIRMED SAFE
  (does not drop an already-associated link) - never arms a dead-man. A
  **numeric** `distance` (unlike the named `"dynamic"`/`"indoors"` modes)
  directly sets the ACK-timeout/TDMA slot timing - **LOCKOUT-RISK**,
  CONFIRMED this can silently drop an already-associated link on a
  mismatch, with no protocol-level recovery of its own - now arms a
  dead-man by default, same mechanism as `set_wireless_channel`/
  `set_wireless_tx_power`, restoring the prior `distance`. CONFIRMED LIVE:
  `adaptive-noise-immunity=ap-and-client-mode` measurably helped with good
  signal but poor CCQ (interference, not distance); an explicit `distance`
  (e.g. 9 for a ~9km link) beat `"dynamic"` for ACK timeout on a long PtP
  link.
- `get_wireless_link_quality(interface=None)` (read) - normalizes the same
  registration-table `wireless_registrations` already reads (ROS7
  `/interface/wifi` first, falls back to ROS6/legacy
  `/interface/wireless` - unchanged fallback, now factored into a shared
  `server._wireless_registration_rows` helper so this doesn't duplicate
  it) into `signal_strength`/`signal_to_noise`/`tx_ccq`/`rx_ccq`/
  `tx_rate`/`rx_rate`/`distance`/`uptime` per peer
  (`formatting.normalize_wireless_registration`) - the numbers that
  actually diagnose a PtP link. A field the current generation's
  registration-table doesn't publish (ROS7's newer `/interface/wifi`
  registration-table does not carry `signal-to-noise`/`tx-ccq`/`rx-ccq`/
  `distance` at all, per MikroTik's own docs) comes back `None`, never
  fabricated.

**Which menu, and why this is `/interface/wireless`, not `/interface/wifi`:**
every field name given by today's live session (`tx-power-mode`,
`frequency-mode=superchannel`, `channel-width`, and a registration-table
that genuinely returns `signal-to-noise`/`tx-ccq`/`rx-ccq`/`distance`)
matches `/interface/wireless` (RouterOS's legacy wireless menu) exactly and
does not exist (or exists in an incompatible shape - nested
`channel.frequency`/`channel.width`, no `tx-power-mode` equivalent at all)
on ROS7's newer `/interface/wifi` package. This is strong, direct evidence
this fleet's PtP radios run the legacy `wireless` package today, even on
ROS 7.21.5 - see `docs/api-notes-wireless-rf.md` for the full reasoning.
`/interface/wifi` write support for channel/tx-power is explicitly out of
scope this round (see `ROADMAP.md`'s updated "Richer ROS7 Wi-Fi" note).

**New validators** (`validation.py`): `validate_wireless_frequency`
(2192-6100MHz, one broad shape-only range spanning 2.4GHz through 5GHz +
superchannel), `validate_wireless_channel_width` (RouterOS's fixed 20-value
enum, case-insensitive), `validate_wireless_tx_power` (-30..40dBm, RouterOS's
own documented range), `validate_adaptive_noise_immunity`,
`validate_wireless_distance` (`"dynamic"`/`"indoors"`/1-1000km),
`validate_dead_man_minutes` (1-60), `validate_dead_man_name` (the
`"deadman-<hex>"` safety-boundary shape), `validate_revert_command`/
`validate_revert_commands` (non-empty, max 10 items, no control
characters, a fixed set of script-structure-breaking characters, a
positive `set [find ...]` structural allowlist, and a redundant verb
denylist - see above).

**New formatter** (`formatting.py`): `format_ros_date_time(datetime) ->
(date, time)`, the inverse of the existing `parse_ros_datetime` - formats a
Python `datetime` into RouterOS's own `date`/`time` field shapes
(`"jul/13/2026"`/`"14:23:05"`), used by `guard._dead_man_deadline` to
compute a dead-man's one-shot `start-date`/`start-time`.

**Tests**: `tests/fakes.py` gains a simulated device clock
(`FakeConnection`'s `("system", "clock")` row, defaulted for every fake
unless a test overrides it) and `advance_clock(minutes)`/
`fire_scheduler(name)`. `fire_scheduler` now refuses to fire (raises
`AssertionError`) unless the fake's simulated clock has reached the
scheduler's `start-date`/`start-time` - proving a dead-man's deadline is
genuinely in the future, not "now" - and mirrors RouterOS's
abort-on-first-parse-error behavior: it stops at the first on-event
statement it doesn't recognize, never running anything after it (including
the self-remove), proving a broken revert command leaves a stale scheduler
entry behind instead of silently completing. Key tests added: the dead-man
arms *before* the risky write is sent (proven by spying on `client.update`
and asserting the scheduler row already exists at write time); a dead-man
does not fire before its deadline; firing after the deadline restores the
interface's prior state and self-removes; cancelling prevents the revert;
`cancel_dead_man` rejects a name outside its own shape before ever reading
the device; a confirmed `set_wireless_channel` call journals TWO audit
entries (the write, and the nested `arm_dead_man` call) - see
`test_guard_audit.py`; the scheduler deadline computation rolls over
midnight/month boundaries correctly; `arm_dead_man` raises if the device's
`/system/clock` can't be read or parsed.
